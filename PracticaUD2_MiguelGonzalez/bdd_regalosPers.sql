...
DROP DATABASE IF EXISTS bbdd_tienda_regalos_personalizados;
CREATE DATABASE bbdd_tienda_regalos_personalizados;
USE bbdd_tienda_regalos_personalizados;
...
CREATE TABLE camiseta (
    ...
    precio_unitario DECIMAL(8,2) DEFAULT 15.50,
...
CREATE TABLE taza (
    ...
    cantidad INT NOT NULL, 
    precio_unitario DECIMAL(8,2) DEFAULT 10.00,
    FOREIGN KEY (idenvio) REFERENCES envios(idenvio) ON DELETE CASCADE
);
...
CREATE TABLE llavero (
    ...
    cantidad INT NOT NULL, 
    precio_unitario DECIMAL(8,2) DEFAULT 5.00,
    FOREIGN KEY (idenvio) REFERENCES envios(idenvio) ON DELETE CASCADE
);
...
BEGIN
    ...
    SELECT IFNULL(SUM(cantidad * precio_unitario), 0) INTO t_cam FROM camiseta WHERE idenvio = p_idenvio;
    SELECT IFNULL(SUM(cantidad * precio_unitario), 0) INTO t_taz FROM taza WHERE idenvio = p_idenvio;
    SELECT IFNULL(SUM(cantidad * precio_unitario), 0) INTO t_lla FROM llavero WHERE idenvio = p_idenvio;
...
SELECT 
    ...
    e.nombre_completo, 
    e.dni_cliente,
    calcular_total_envio(e.idenvio) AS total_pagar,
    e.estado
FROM envios e;